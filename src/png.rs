...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(value: &[u8]) -> Result<Self> {
        ...
        if header != Png::STANDARD_HEADER {
            return Err(PngDecodeError::boxed(format!("Bad header given {:?} expected {:?}", header, Png::STANDARD_HEADER)));
        }
        let mut chunks = Vec::new();
        let mut len_buf: [u8; 4] = [0; 4];
        while let Ok(()) = reader.read_exact(&mut len_buf) {
            //4 extra bytes for the chunk type and then data and then 4 more for crc
            let final_pos = 4 + u32::from_be_bytes(len_buf) + 4;
            let mut data = vec![0; usize::try_from(final_pos)?];
            reader.read_exact(&mut data)?;
            let bytes: Vec<u8> = len_buf.iter().copied().chain(data.into_iter()).collect();
            let chunk = Chunk::try_from(bytes.as_slice())?;
            chunks.push(chunk);
        }
...
impl Png {
    ...
    fn remove_chunk(&mut self, chunk_type: &str) -> Result<Chunk> {
            ...
            .position(|c| c.chunk_type() == &chunk_type)
        {
            ...
            Err(ChunkNotFoundError::boxed(format!("{}", chunk_type)))
        }
    }
    fn header(&self) -> &[u8; 8] {
...
impl Display for ChunkNotFoundError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(f, "Chunk not found with type {}", self.reason)
    }
}
